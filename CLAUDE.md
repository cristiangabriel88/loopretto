...
- **Drone (§5.37)**: its own top-level panel (the "Drone" toggle button next to Sound/Practice/Metronome/Piano, panel `#drone-body`), **not** part of the Practice panel. Up to three sustained sine oscillator voices (note + octave, optional octave-below) feed one lowpass → Vol gain → `destination`, **bypassing `masterGain`** so memorize/transpose don't affect the reference pitch. `setDrone(false)` is the teardown hook.
